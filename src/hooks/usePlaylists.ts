...
import { getSnapshot, subscribe } from "../api/cacheStore";
import {
  PLAYLISTS_KEY,
  createPlaylist,
  deletePlaylist,
...
export function usePlaylists() {
  const playlists = useSyncExternalStore(
    ...
    () => getSnapshot<Playlist[]>(PLAYLISTS_KEY, []), // ✅ [] stabile
    () => getSnapshot<Playlist[]>(PLAYLISTS_KEY, []),
  );
  ...
  const isFavorite = (songId: string) =>
    playlists.find((p) => p.id === FAVORITES_ID)?.songIds.includes(songId) ??
    false;

  const isSongInPlaylist = (playlistId: string, songId: string) =>
    playlists.find((p) => p.id === playlistId)?.songIds.includes(songId) ??
    false;
